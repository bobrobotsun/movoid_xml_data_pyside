...
class MainWindow(QMainWindow):
    ...
    def init_tab_label(self):
        ...
        label_group = QGroupBox('label', self.tab_label)
        grid.addWidget(label_group, 0, 0)
        body_group = QGroupBox('body', self.tab_label)
        grid.addWidget(body_group, 0, 1)
        now_group = QGroupBox('now', self.tab_label)
        grid.addWidget(now_group, 0, 2)

        label_layout = QGridLayout(label_group)
        label_tree = QTreeWidget(label_group)
        label_tree.setObjectName('label_label_tree')
        label_layout.addWidget(label_tree)
        label_tree.setHeaderLabels(['key', 'value'])

        body_layout = QGridLayout(body_group)
        body_list = QListWidget(body_group)
        body_list.setObjectName('label_body_list')
        body_layout.addWidget(body_list)

        now_layout = QGridLayout(now_group)
        now_tree = QTreeWidget(now_group)
        now_tree.setObjectName('label_now_tree')
        now_layout.addWidget(now_tree)
        now_tree.setHeaderLabels(['key', 'value'])

    def init_tab_body(self):
        self.tab_body = QWidget(self)
        grid = QGridLayout(self.tab_body)
        grid.setColumnStretch(0, 1)
        grid.setColumnStretch(1, 5)
        grid.setColumnStretch(2, 2)
        self.tab_body.setLayout(grid)
        label_group = QGroupBox('label', self.tab_body)
        grid.addWidget(label_group, 0, 0)
        body_group = QGroupBox('body', self.tab_body)
        grid.addWidget(body_group, 0, 1)
        now_group = QGroupBox('now', self.tab_body)
        grid.addWidget(now_group, 0, 2)

        label_layout = QGridLayout(label_group)
        label_list = QListWidget(label_group)
        label_list.setObjectName('body_label_list')
        label_layout.addWidget(label_list)

        body_layout = QGridLayout(body_group)
        body_tree = QTreeWidget(body_group)
        body_tree.setObjectName('body_body_tree')
        body_layout.addWidget(body_tree)
        body_tree.setHeaderLabels(['key', 'value'])

        now_layout = QGridLayout(now_group)
        now_tree = QTreeWidget(now_group)
        now_tree.setObjectName('body_now_tree')
